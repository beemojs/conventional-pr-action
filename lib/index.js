...
function getPath(part) {
    return path_1.default.join(CWD, part);
}
...
function detectPackageManager() {
    ...
    if (fs_1.default.existsSync(getPath('yarn.lock'))) {
        pm = 'yarn';
    }
    else if (fs_1.default.existsSync(getPath('pnpm-lock.yaml'))) {
        pm = 'pnpm';
    }
    else {
        pm = 'npm';
...
function isYarn2AndAbove() {
    return fs_1.default.existsSync(getPath('.yarn')) || fs_1.default.existsSync(getPath('.yarnrc.yml'));
}
function getPackageJson() {
    return JSON.parse(fs_1.default.readFileSync(getPath('package.json'), 'utf8'));
}
async function installPackages() {
    const bin = detectPackageManager();
    core_1.startGroup(`Installing dependencies with ${bin}`);
    if (bin === 'yarn' || bin === 'pnpm') {
        await exec_1.exec(bin, ['install', isYarn2AndAbove() ? '--immutable' : '--frozen-lockfile'], {
            cwd: CWD,
        });
    }
    else {
        await exec_1.exec('npm', ['ci'], { cwd: CWD });
    }
    core_1.endGroup();
}
async function installPresetPackage(name, version) {
    ...
    core_1.startGroup(`Installing preset package ${pkg}`);
    // Yarn
    if (bin === 'yarn') {
        if ('workspaces' in getPackageJson() && !isYarn2AndAbove()) {
            core_1.info('Workspaces detected, installing to root');
            args.push('-W');
        }
        await exec_1.exec('yarn', args, { cwd: CWD });
        // PNPM
    }
    else if (bin === 'pnpm') {
        if (fs_1.default.existsSync(getPath('pnpm-workspace.yaml'))) {
            core_1.info('Workspaces detected, installing to root');
            args.push('-W');
        }
        await exec_1.exec('pnpm', args, { cwd: CWD });
        // NPM
    }
    else {
        await exec_1.exec('npm', args, { cwd: CWD });
    }
    core_1.endGroup();
}
async function run() {
    try {
        ...
        const autoInstall = core_1.getInput('auto-install');
        if (autoInstall) {
