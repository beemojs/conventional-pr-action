...
async function run() {
    try {
        ...
        try {
            await io_1.which('yarn', true);
            await exec_1.exec('yarn', ['add', preset]);
        }
        catch (_a) {
            await exec_1.exec('npm', ['install', preset]);
        }
        // Verify the PR title against the preset
        const config = conventional_changelog_preset_loader_1.default(core_1.getInput('config-preset'));
        let result = null;
